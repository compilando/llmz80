...
def _candidate(
    ...
    *,
    ...
    (path / "semantic_report.json").write_text(
        json.dumps(
            {
                "quality_pass": semantic,
                "errors": [],
                "warnings": [],
            }
        ),
    ...
    (path / "emulator_report.json").write_text(
        json.dumps(
            {
                "requested_full": runtime_required,
...
def test_candidate_selection_prefers_quality_then_smaller_binary(tmp_path):
    ...
    assert [item["run_dir"] for item in report["candidates"]] == [
        str(second),
        str(first),
        str(broken),
    ]
