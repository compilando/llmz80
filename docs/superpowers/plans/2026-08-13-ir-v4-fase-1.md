...
- **La Tarea 8 no se ejecutó, y la Tarea 10 dio por hecho que sí.**
  `resources/studio_lib/common/platform.h` sigue definiendo `IN_LEFT`..`IN_ACTION`
  y `CELL_EMPTY`..`CELL_WALL`, y declara `plat_cell(col, row, unsigned char kind)`;
  las dos `platform.c` siguen leyendo las cinco teclas fijas de v3 y dibujando
  cuatro formas cableadas (`shape_player`, `shape_enemy`, ...). `tests/test_platform_library.py`
  no existe. Consecuencias medibles hoy:
  - El prompt del escritor se contradice a sí mismo: `acceptance.py` le dice
    *"Draw one with plat_cell(col, row, character)"* y, unas líneas más abajo, el
    propio `platform.h` que se le adjunta declara `plat_cell(..., unsigned char kind)`
    con `CELL_PLAYER`/`CELL_ENEMY`/`CELL_COLLECTIBLE`.
  - `game_config.h` emite `INPUT_JUMP` y la X-macro `INPUT_BINDINGS`, pero
    `plat_input()` no la expande: devuelve `IN_LEFT`/`IN_RIGHT`/`IN_UP`/`IN_DOWN`/`IN_ACTION`,
    así que SPACE vuelve como `IN_ACTION` (0x10) y no como `INPUT_JUMP` (0x04).
    El bit de salto existe en el header y sólo lo alcanza un programa que lea las
    teclas por su cuenta, sin usar la librería.
  Es la primera tarea que debe entrar en la fase 2, antes que el examinador.
