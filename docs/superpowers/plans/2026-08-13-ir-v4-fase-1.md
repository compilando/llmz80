...
> **Ejecutada. El código de abajo es el punto de partida, no lo entregado.** La revisión de calidad encontró tres corsés v3 que este bloque aún conservaba, y el arreglo está en `2f4e1f7`: los efectos de audio eran un `Literal` cerrado que decía `collect` y `level`; `SceneKind` era un enum cerrado sin ningún lector semántico tras la Tarea 6; y `TargetSpec` había perdido el invariante plataforma↔modo de vídeo, de forma que `spectrum` + `cpc_mode_0` se aceptaba y producía un binario roto sin un solo error. Lee `git show eb11259 2f4e1f7` para el estado real de `models.py`. Las Tareas 3, 7 y 8 de este documento ya están escritas contra la versión arreglada.
...
> **Ejecutada. El código de abajo es el punto de partida, no lo entregado.** La revisión de calidad encontró que `FIELD_TOP = 2` reservaba dos filas de HUD a todo diseño —el mismo corsé que los roles de v3, en forma aritmética— y que faltaban tres comprobaciones de auto-referencia, dos de las cuales quedaron luego sin test hasta que un test de mutación las delató. El estado real está en `git show 999e4b9 fdab7b4 734fccb 06945f9 bccbbd0`: las filas del HUD las decide `presentation.hud_rows`, `structural_errors` se reparte en `_identity_errors`/`_reference_errors`/`_fit_errors`, y el documento en blanco ya no acuña `player` ni escribe mecánicas por su cuenta.
...
Nota: `kind` es una cadena, no un enum. La Tarea 1 borró `SceneKind` tras comprobarse que su único consumidor semántico (`quality.py`) desaparece en la Tarea 6, y que `LEVEL_COMPLETE` presuponía los niveles que v4 no tiene. Estos tres valores son los que este documento en blanco elige, no los que Studio permite.
...
### Task 5b: `registry.py` — deja de haber un registro de géneros

> **Tarea añadida durante la ejecución.** El plan original no la tenía y era un hueco real: `registry.py` importa `from .packs import BUILTIN_PACKS, GenrePack`, y la `quality.py` de la Tarea 6 importa `audio_gaps` y `target_registry` de `registry`. Sin esto, la Tarea 6 no puede ni importar su propio módulo.

**Files:**
- Modify: `llmz80/studio/registry.py:11, 105-135`
- Modify: `llmz80/studio/plugins.py:18`
- Rewrite: `examples/studio_plugin/retro_bonus_pack.py`, `examples/studio_plugin/pyproject.toml`
- Modify: `tests/test_studio_registry_planner.py`
- Modify: `docs/STUDIO_EXTENSIONS.md`

- [ ] **Step 1: Escribe el test que falla**

En `tests/test_studio_registry_planner.py`, borra `test_builtin_genre_registry_has_stable_ids` y añade:
...
Run: `.venv/bin/python -m pytest tests/test_studio_registry_planner.py -q`
Expected: FAIL — `ModuleNotFoundError: No module named 'llmz80.studio.packs'`.

- [ ] **Step 3: Limpia `llmz80/studio/registry.py`**

Borra la línea `from .packs import BUILTIN_PACKS, GenrePack`, la clase `TargetPlugin` si ha quedado sin uso, la función `genre_registry` entera, y la importación de `GENRE_PACK_GROUP`. El módulo se queda con `TargetPack`, `audio_gaps`, `Registry`, `BUILTIN_TARGETS` y `target_registry`. Añade sobre `BUILTIN_TARGETS` la razón:
...
- [ ] **Step 4: Quita el grupo de puntos de entrada de géneros**

En `llmz80/studio/plugins.py`, borra `GENRE_PACK_GROUP = "llmz80.genre_packs"`.

- [ ] **Step 5: Reescribe el ejemplo del SDK**

`examples/studio_plugin/retro_bonus_pack.py` registra hoy un `GenrePack`, que ya no existe. Sustitúyelo por lo único de ese fichero que Studio carga de verdad — un target de terceros:
...
En `examples/studio_plugin/pyproject.toml`, cambia el grupo del punto de entrada de `llmz80.genre_packs` a `llmz80.target_plugins`.

- [ ] **Step 6: Actualiza `docs/STUDIO_EXTENSIONS.md`**

Run: `grep -n 'genre\|Genre' docs/STUDIO_EXTENSIONS.md`

Borra la sección que enseña a añadir un género y sustitúyela por una frase: un género ya no es una extensión porque ya no es nada — el diseño declara su propio vocabulario, y lo que antes se añadía como pack ahora se escribe en el `brief` del proyecto.
...
```bash
git add llmz80/studio/registry.py llmz80/studio/plugins.py examples/studio_plugin docs/STUDIO_EXTENSIONS.md tests/test_studio_registry_planner.py
git commit -m "refactor(studio): stop registering kinds of game beside kinds of machine"
```
...
Borra el bloque de `#define IN_LEFT 0x01` a `#define IN_ACTION 0x10`, el bloque de `#define CELL_EMPTY 0` a `#define CELL_WALL 4`, y el bloque de `#define SOUND_START 0` a `#define SOUND_GAME_OVER 4`.

Los `SOUND_*` se van porque ahora los emite `game_config.h` con los nombres que el diseño acuñó (`SOUND_DIG`, `SOUND_FALL`), y dejarlos aquí sería una segunda definición del mismo símbolo. Sobre `plat_sound`, deja escrito el contrato que queda:
...
En su lugar del bloque de entrada, sobre `void plat_init(void);`:
...
Y sustituye el `switch` de `plat_sound` (líneas 154-163), que hoy despacha sobre `SOUND_COLLECT`, `SOUND_HIT` y compañía — constantes que ya no existen, porque `game_config.h` sólo define las que el diseño acuñó:
...
### Task 10b: el contrato de estado deja de exigir marcador, vidas y niveles

> **Tarea añadida durante la ejecución.** La revisión de la Tarea 10 encontró que borrar la derivación de bolitas no quitó el corsé: lo movió al prompt. `llmz80/core/state_contract.py` no lo toca ninguna tarea del plan, y `generation_prompt` lo grapa delante de cada diseño.

El problema, en las palabras del propio fichero: `g_score`, `g_lives` y `g_level` son `required=True`, y `g_remaining` significa "objectives still to clear on this level". Un puzzle sin marcador, sin vidas y sin niveles debe declarar los tres igual, y es lo primero que lee el escritor. Peor: el significado de `g_lives` dice *"lives left; the design states the starting value"* — y v4 borró `gameplay.lives`, así que el contrato manda consultar un campo que ya no existe.

**Files:**
- Modify: `llmz80/core/state_contract.py:34-63`
- Modify: `llmz80/studio/generator.py:136-140`
- Modify: `tests/test_runtime_contracts.py`, `tests/test_studio_probes.py`

- [ ] **Step 1: Escribe el test que falla**

En `tests/test_runtime_contracts.py`:
...
Expected: FAIL — hoy `REQUIRED_SYMBOLS` son cinco.

- [ ] **Step 3: Baja `g_lives` y `g_level` a opcionales**

En `STATE_CONTRACT`, `g_lives` y `g_level` pasan a `required=False`, y sus significados dejan de presuponer el juego que fueron:
...
```python
    ...
    StateSymbol(
        ...
        1,
        False,
        "current level or screen number, counting from one, when the game "
        "advances through them",
    ),
```

Y en `contract_prompt`, la frase que introduce los opcionales pasa a decir que un diseño que no tenga esa noción **no debe declarar** el símbolo, en vez de dejarlo ambiguo.

- [ ] **Step 4: Arregla la promesa muerta en `generator.py`**

`generator._instructions` dice hoy: *"the screen after **the scripted inputs** is compared against the screen before them"*. Tras la Tarea 10 no hay inputs guionados hasta la fase 2, así que promete una puerta que no dispara. Y en la misma frase pide dibujar "the playfield, the actors and **the score**", que es corsé.

Sustituye ese párrafo por uno que diga lo que de verdad se comprueba: que el programa dibuje, que la pantalla cambie cuando el juego avanza, y que dibuje lo que su propio diseño declara — sin nombrar marcador ni actores como obligatorios.

- [ ] **Step 5: Corre los tests**

Run: `.venv/bin/python -m pytest tests/test_runtime_contracts.py tests/test_studio_probes.py -q`
Expected: PASS. Si `test_studio_probes.py` afirma que faltan símbolos requeridos que ya no lo son, pórtalo.

- [ ] **Step 6: Commit**

```bash
git commit -m "refactor(core): stop requiring a score, lives and levels of every program"
```
...
**Files:**
- Modify: `llmz80/studio/services.py:19-56`, **`:304-346`** y `:416-500`
- Modify: `tests/test_studio_probes.py`
...
**Borra también `SWEEP_KEYS` y `scenario_script` (`:304-346`).** Son la derivación de bolitas literal, sobrevivida a la Tarea 10: `scenario_script` lee `project.controls.scheme`, que v4 no tiene, así que `runtime_test` levanta `AttributeError` antes siquiera de llegar al guión vacío. `runtime_test` pasa a llamar a `smoke_test(..., script=[])` directamente. Y en `acceptance_report`, la razón de la abstención dice hoy `"this design states no executable acceptance scenario"`, que culpa al diseño de que Studio no tenga examinador todavía: redáctala como la de `probe_report`, nombrando la causa real.
...
- **`project.campo = x` no funciona en v4, y parece que debería.** `models.StrictModel` lleva `validate_assignment=True` y `validate_structure` valida el proyecto entero, así que cualquier asignación de atributo revalida todo el documento — y un renombrado no se puede hacer por asignación **en ningún orden**: asignar `screens` primero falla porque `initial_screen` aún apunta al id viejo, y al revés falla por lo mismo. `editing.py` ya trabaja dict adentro / `model_validate` afuera vía `_document`, y las Tareas 11 y 12 deben seguir haciéndolo. No es un defecto que arreglar, es una consecuencia de validar el conjunto; está escrito aquí porque el atajo es tentador.
- **Las filas del HUD las decide el diseño**, no Studio: `presentation.hud_rows` (default 2). `structure.playfield` las resta y `game_config.h` las emite. Si ves un `FIELD_TOP` constante en `codegen.py`, `compiler.py` o la librería C, es de v3 y su tarea lo porta.
...
## Ejecución, 2026-08-14

Ejecutado: Tareas 1, 2, 3, 4, 5, 5b, 6, 7, 9, 10, 10b, 11, 12 y 13.
**Sin ejecutar: la Tarea 8** (ver abajo). Suite en verde — 586 tests, cero
errores de colección — y `make lint` limpio. La evidencia de aceptación de la
fase es `studio-projects/fase-uno`: valida y reabre con dos pantallas unidas por
`exits`, una entidad de `kind: perseguidor`, un binding `jump: SPACE` junto a
`left`/`right`, y un tercer tile con su propio trait `climbable`. Ninguna de las
cuatro cosas obligó a relajar una validación para pasar.

### Lo que la ejecución encontró y el plan no preveía

1. **Un registro de géneros que ninguna tarea tocaba.** `registry.py` mantenía
   un catálogo de tipos de juego junto al de plataformas, así que el vocabulario
   fijo sobrevivía al borrado de `typologies.py`. Se añadió la Tarea 5b.
2. **`FIELD_TOP = 2` era el corsé en forma aritmética.** El andamio reservaba dos
   filas de HUD a todo diseño, incluido uno sin marcador. Las filas las decide
   ahora `presentation.hud_rows`.
3. **El contrato de estado exigía marcador, vidas y niveles.** `state_contract.py`
   no aparecía en ninguna tarea, y `generation_prompt` lo grapa delante de cada
   diseño: era lo primero que leía el escritor. Peor, el significado de `g_lives`
   mandaba consultar `gameplay.lives`, un campo que v4 había borrado. Se añadió
   la Tarea 10b: sólo `g_score` y `g_state` siguen siendo obligatorios.
4. **`services.scenario_script` sobrevivió a su propio borrado.** La Tarea 10
   quitó la derivación de escenarios de `acceptance.py`, pero el barrido de
   bolitas seguía vivo en `services.py`, guionizando el mismo juego de siempre
   contra cualquier diseño.
5. **Huevo y gallina en `draw_sprites`.** Ninguna entidad podía estrenar un
   sprite: dibujar exigía que la entidad ya llevara un `sprite` id, y el id sólo
   se asignaba al dibujar. Arreglado en `2b30acc`.

### Deuda que esta fase deja abierta

- **La Tarea 8 se ejecutó al final, fuera de orden** (se saltó al reordenar las
  tareas por cuello de botella de colección, y la Tarea 13 la encontró ausente).
  Cerrada en `b642de3`: la librería lee la entrada por `INPUT_BINDINGS`, dibuja
  terreno con `plat_cell(col, row, char)` y despacha sonidos por índice, con
  compilación real verificada en las dos máquinas — `output.tap` de 4449 bytes
  con `zcc` y `output.dsk` con CPCtelera. De paso destapó dos cosas que ninguna
  tarea preveía: `platform.h` y `game_config.h` definían ambos `FIELD_TOP`, una
  redefinición de macro que la política de warnings habría rechazado en cuanto
  un diseño pidiera `hud_rows` distinto de 2; y el programa de referencia de
  `resources/studio_reference/` —el ejemplo trabajado que `retrieval.py` le
  enseña al escritor— seguía en `schema_version: 3` y usaba `START_LIVES`,
  `SCORE_PER_COLLECTIBLE`, `LEVEL_COUNT` y `DIFFICULTY_CURVE`, constantes que
  `game_config.h` dejó de emitir.
- `studio-projects/` está en `.gitignore`, así que ni el archivo v3 ni
  `fase-uno` quedan versionados: el archivado de la Tarea 13 se hizo con `mv`,
  no con `git mv`, y su `README.md` vive sólo en disco.

## Lo que la revisión final dejó anotado para la fase 2

Tres defectos se arreglaron en `acf7669` (la guarda de límites de `set_tile`, el
`sorted` sobre `sprite` nulo en la TUI, y `game_state.h` contradiciendo al
contrato en el mismo prompt). Lo que queda, por orden de lo que más va a doler:

1. **El ejemplo trabajado que encabeza el prompt del escritor es un comecocos.**
   `retrieval.reference_program()` mete `resources/studio_reference/<target>/src/engine.c`
   el primero, etiquetado como programa que satisface el contrato: 270 líneas de
   laberinto con `KIND_COLLECTIBLE` y `SCORE_PER_COLLECTIBLE`. No rechaza nada,
   pero es la demostración autorizada de qué es un juego, y pesa más que
   cualquiera de los seis corsés que esta fase quitó. La evidencia de aceptación
   de F2 es rehacer `profanacion`: esto hay que arreglarlo **antes** de esa
   tirada, no después. Su README además niega que ese fichero llegue al prompt.
2. **`emulator_smoke._run_zesarux` acepta una tecla por paso.** El `exam.yml` que
   el spec define necesita `hold: {right: 40, jump: 6}` — pulsaciones compuestas,
   que es justamente lo que hacía inexaminable a un plataformas. Ese bucle y
   `feel._classify` se reescriben, no se adaptan.
3. **`feel.py` y `generator.repair_prompt` están escritos contra el esquema
   borrado**: citan `AcceptanceScenario` y `ScenarioHold`, y leen la forma
   escalar `scenario['hold']`. Ambos son código inalcanzable hoy (las puertas se
   abstienen), así que nadie lo notó al portar.
4. **Studio no sabe escribir el vocabulario que ahora expresa.** No hay
   `add_tile`, `add_screen`, `set_exit`, `set_binding`, `set_mechanics` ni
   `add_observable`, y `tui.render_map` sólo pinta muro y suelo: un tercer tile
   es invisible en el editor. `fase-uno` se editó a mano, y el `profanacion` de
   F2 saldrá del camino de propuestas de IA o de YAML a mano.
5. **La CLI vomita tracebacks donde esta fase escribió mensajes honestos.**
   `cli.py` sólo captura `FileNotFoundError`, así que abrir un proyecto v3
   archivado enseña un stack de pydantic en vez del mensaje que la Tarea 5
   redactó a propósito.
6. Código muerto que conviene retirar antes de que alguien lo crea vivo:
   `planner.apply_proposal(allow_unplayable=...)`, `registry.TargetPack.validate`
   (que además discrepa de `quality` en el presupuesto de datos), y el cuerpo de
   juicio de `services.acceptance_report` tras su `if not steps`.
7. Sin test: el historial de revisiones y el guardado atómico de `store.save`
   —seguridad de datos del usuario— no los protege nada.
