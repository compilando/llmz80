...
Entre los dos no hay nadie. Por eso `studio-projects/zampabolas` y `studio-projects/my-retro-game` llegaron al escritor con `mechanics: []` — el segundo desde una ficha que había identificado correctamente *Harrier Attack!* de Durell con ocho fuentes citadas — y por eso `llmz80 make` se detiene hoy en `programa` cuando la investigación no identifica nada.
...
1. **El redactor inventa un juego distinto del que pide el brief.** Es el riesgo que el prompt de `adapt` evita no redactando nunca, y esta etapa lo asume a propósito. La defensa natural sigue siendo el examinador de la tarea 8 del plan anterior (`design_exam.py`), que ya compara diseño contra brief y ya está enchufado al bucle de `adapt`.
