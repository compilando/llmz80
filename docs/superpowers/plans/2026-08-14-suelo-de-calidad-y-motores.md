...
| **M10** | **Decidir quién redacta el diseño a partir del brief.** Salido de las tareas 7 y 8. `adapt` no redacta: su propio prompt dice que «el diseño ya decidió qué es este juego -- el `kind` de cada entidad, las mecánicas que declara y cómo se conectan sus pantallas están decididos y no son tuyos para cambiarlos». Viste un diseño existente con la ropa de un juego real. Y `blank_project` da una entidad `actor` y dos tiles. Entre los dos no hay nadie que convierta «un avión de combate con scroll y cazas enemigos» en entidades y mecánicas. `/mechanics` sí es una ruta proponible y `PROTECTED_PATHS` sólo protege cuatro rutas, así que añadir entidades está permitido por el aplicador aunque el prompt lo desaconseje: la limitación es de doctrina, no de mecanismo. Hay que elegir — o `adapt` redacta cuando no hay nada que vestir, o nace una etapa de redacción antes de él. Mientras no se elija, un brief que pida una entidad que el proyecto en blanco no tiene agota los intentos del examinador y `make` se detiene con el hueco nombrado | Gap 1, 9 | 2-3 sem | — | Es una decisión de doctrina antes que de código: `adapt` se escribió deliberadamente para no reinterpretar el diseño, y ampliarlo reabre el fallo que ese prompt existe para evitar |
| **M7** | **Que la biblioteca sea la dueña de `g_worst_frame_cost`.** Salido de la revisión de la tarea 6: `studio-projects/fase-uno/program/main.c:69` hace `g_worst_frame_cost = plat_wait_frame()`, que guarda el coste de la *última* iteración y no el peor. Un programa así **nunca puede fallar** la puerta de ritmo, así que el texto de reparación que explica qué debe contener el símbolo es estructuralmente inalcanzable justo en el caso para el que se escribió. La cura no es más prosa: es quitarle al programa la ocasión de equivocarse. Que `plat_wait_frame` mantenga él mismo el máximo y defina el símbolo, y el programa no lo toque. Encaja con la distinción que la tarea 5 escribió en el contrato — `g_score` y `g_state` son exigencias al *diseño*, `g_worst_frame_cost` es una exigencia a la *instrumentación* — y una exigencia a la instrumentación la debe cumplir la instrumentación | Gap 1, 6 | 1 sem | — | Rompe el build de todo programa existente por definición duplicada, y toca el contrato y el prompt del escritor. Barato en código, caro en propagación |
| **M8** | **Decidir si `pacing` cuenta para `VERIFICATION_OBSERVED`.** `quality.BEHAVIOUR_GATES` lo incluye, así que hoy la puerta que menos evidencia exige basta por sí sola para certificar que a un juego «se le vio funcionar» y abrir el release. Con M7 hecho pasa a ser evidencia real; sin M7, es la aprobación más barata del sistema | Gap 1 | 1 d | M7 | Ninguno técnico; es una decisión, no un cambio |
| **M9** | **Un solo predicado `has_frame_clock(platform)`.** `pacing._FRAME_CLOCK_PLATFORMS` y `codegen.render_config_header`'s `HAS_FRAME_CLOCK` deciden lo mismo por separado y nada los ata. El día que el CPC tenga contador de frames, la deriva es silenciosa y en la dirección mala: el C empieza a medir de verdad y la puerta sigue absteniéndose | — | 1 d | — | Bajo |
| **M6** | **Un testigo de movimiento en el contrato de estado.** Salido de la revisión de la tarea 4: `feel.animation_report` afirma que la animación avanza *mientras el jugador se mueve*, pero lo único que puede leer es qué tecla estaba pulsada. Un jugador pegado al borde de la pantalla no se mueve aunque la tecla siga hundida, así que la puerta acaba juzgando la entrada y llamándolo movimiento. La tarea 4 lo tapa con una heurística (exigir que la pareja comparada cruce un cambio de dirección) que nunca falla en falso pero sí deja pasar casos. Un `g_player_x`/`g_player_y` opcional, o un contador `g_moved`, permitiría enunciar la afirmación exacta: *entre estas dos lecturas la posición cambió, luego el fotograma debe diferir* | Gap 1, 6 | 2 sem | M1 | Toca el contrato, el prompt del escritor y `probe_report`, y obliga a regenerar todo programa existente. Barato en código, caro en propagación |
